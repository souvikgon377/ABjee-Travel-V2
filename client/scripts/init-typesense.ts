...
// Load .env file BEFORE importing runtime modules so they pick up env vars.
dotenv.config({ path: path.join(rootDir, '.env') });

async function run() {
  // Dynamic import to ensure env is loaded first
  const { initializeTypesense, healthCheckTypesense } = await import('../src/modules/search/typesenseClient');
  ...
  try {
    ...
    console.log(`📡 Checking Typesense connectivity at ${process.env.TYPESENSE_HOST || process.env.TYPESENSE_URL || 'localhost'}:${process.env.TYPESENSE_PORT || 8108}...`);
    const isHealthy = await healthCheckTypesense(10_000);
    
    if (!isHealthy) {
      throw new Error(
        `Typesense is not reachable. Make sure it's running at ${process.env.TYPESENSE_HOST || process.env.TYPESENSE_URL || 'localhost'}:${process.env.TYPESENSE_PORT || 8108}. ` +
    ...
    console.error(`   2. Check .env file has TYPESENSE_HOST, TYPESENSE_PORT, TYPESENSE_PROTOCOL, TYPESENSE_API_KEY or TYPESENSE_URL`);
